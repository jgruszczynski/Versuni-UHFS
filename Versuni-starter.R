...
library(googledrive)
pckgver::install_or_update_package("gmailr", "2.0.0")
library(gmailr)
...
gm_auth_configure(path = "/home/jgruszczynski/R-Scripts/!GmailCredentials/ScienceGmailCredentials.json")
gm_auth(token = token)
...
AzureContainer <- storage_container(bl_endp_key,"uhfs")

# Main Loop ----
folder <- "/home/jgruszczynski/R-Scripts/Versuni-UHFS"
setwd(folder)

d1 <- as.Date("2025-06-25", origin = "1970-01-01")
d2 <- as.Date("2025-06-30", origin = "1970-01-01")
